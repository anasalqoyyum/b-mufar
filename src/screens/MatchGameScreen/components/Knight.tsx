...
const ChessSquare = ({ width, position, receptive }: ChessSquareProps) => {
  ...
  return (
    <DraxView
      style={[styles.square, colorStyle, receptive ? styles.receptive : undefined, { width }]}
...
const KnightMoves = () => {
  ...
  for (let row = 0; row < 8; row += 1) {
    ...
    for (let column = 0; column < 8; column += 1) {
      ...
      const receptive = moving && ((rowOffset === 2 && columnOffset === 1) || (rowOffset === 1 && columnOffset === 2))
      squareViews.push(<ChessSquare width={squareWidth} key={`r${row}c${column}`} position={{ row, column }} receptive={receptive} />)
    }
    rowViews.push(
        ...
        <View style={styles.containerRow}>
          ...
          <View style={{ width: boardWidth }}>
            <Text style={styles.instructionText}>
              Start dragging the knight, and the legal move positions will be highlighted with blue borders. When dragging the knight over
              one of those positions, the square will be highlighted with a magenta border instead. Release the drag in a legal position to
              move the knight; release it anywhere else, and it will snap back to its original position.
            </Text>
          </View>
        </View>
      </View>
